import { todoDeleteSchema, TodoResponse } from "@app-store/apps/todos/api-contracts/todo.schema";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const requestBody = todoDeleteSchema.parse(req.body);
    const response: TodoResponse = await entity.delete(requestBody.id, session.user.id);
